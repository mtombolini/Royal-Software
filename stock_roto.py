...
# Función para analizar datos de un SKU específico
def analisis_producto(dataframe, sku):
    df_producto = dataframe[dataframe['SKU'] == sku]
...
# Función para graficar ventas por semana de un SKU específico
def grafico_ventas_por_semana(dataframe, sku):
...
dataframe_1 = preprocesamiento.lectura(parametros.RUTA_SALES_REPORT)
dataframe_1['Fecha Venta'] = pd.to_datetime(dataframe_1['Fecha Venta'], dayfirst=True)

# SKU del producto a analizar
sku_a_analizar = '6010233'

# Ejecutar las funciones de análisis y gráficos
print(analisis_producto(dataframe_1, sku_a_analizar))
grafico_ventas_por_semana(dataframe_1, sku_a_analizar)
